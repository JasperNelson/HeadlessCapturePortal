...
def Intake(pargs: Optional[List[str]] = None) -> argparse.Namespace:
    # in general flags that start with a capital letter change the purpose of the program and cannot be combined with 
    # other capitilized flags.
    ...
    mode = argparser.add_argument_group(
        'MODE:', 'Changes the Mode/Function of the program')
    ...
    modeX.add_argument(
        ...
        type=str,
        default="",
        help="If specified will automatically search through your login files stored in the login directory specified" 
        "in your config file and log you in into a network that matches the URL \n and(if specified) the SSID of the"
        " login file to the current network"
    )
